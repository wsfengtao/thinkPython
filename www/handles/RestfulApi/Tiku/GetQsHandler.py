...
class GetQsHandler(BaseHandler):
    __url__ = r'/api/v1/tiku/list'
    ...
    def __init__(self, application, request, **kwargs):
        ...
        self.start_tihao = self.streng_get_argument("start_tihao", match=r'^[1-9][\d]*$')
        self.size = self.streng_get_argument("size", match=r'^[1-9]\d*$')
    ...
    def prepare(self):
        ...
        if self.request.method == 'GET':
            if self.city and self.cycle_type and self.kemu_type and self.zhangjie and self.start_tihao and self.size \
                    and self.callback:
                pass
            else:
    ...
    @tornado.gen.coroutine
    def get(self):
        r = yield tornado.gen.Task(self.celery.send_task, 'www.celery_tasks.task_rest_tiku.get_qs_list',
                                   args=[self.city, self.cycle_type, self.kemu_type, self.zhangjie, self.start_tihao,
                                         self.size],
                                   queue='queue1')
        if r.state == 'SUCCESS':
